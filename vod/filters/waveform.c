...
typedef struct
{
	...
	uint32_t timescale;
	int64_t last_pts;
} waveform_encoder_state_t;

typedef struct {
	frame_list_part_t cur_frame_part;
	input_frame_t* cur_frame;
	int64_t pts;
} waveform_frame_reader_state_t;
...
typedef struct {
	...
	waveform_frame_t data;
	int64_t flush_pts;

	waveform_frame_reader_state_t reader;
	audio_decoder_state_t* decoder;
} waveform_writer_state_t;
	...
	waveform_frame_t* result)
{
	...
	default:
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"waveform_calc_frame: unsupported sample format %d", frame->format);
...
vod_status_t
waveform_encoder_init(
	request_context_t* request_context,
	uint32_t timescale,
	vod_array_t* frames_array,
	void** result)
{
	waveform_encoder_state_t* state;

	state = vod_alloc(request_context->pool, sizeof(*state));
	if (state == NULL)
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"waveform_encoder_init: vod_alloc failed");
...
{
	waveform_encoder_state_t* state = context;

	media_info->timescale = state->timescale;
	media_info->codec_id = VOD_CODEC_ID_WAVEFORM;
	return VOD_OK;
}
...
// memory frame reader
static void
waveform_frame_reader_init(waveform_frame_reader_state_t* state, media_track_t* track)
{
...
static vod_status_t
waveform_frame_reader_get_frame(
	waveform_frame_reader_state_t* state,
	waveform_frame_t** data,
	int64_t* pts)
{
...
// writer
static vod_status_t
waveform_writer_init_track(waveform_writer_state_t* state)
{
	vod_pool_cleanup_t *cln;
	media_track_t* track = state->cur_track;
	vod_status_t rc;

	if (track->media_info.codec_id == VOD_CODEC_ID_WAVEFORM)
	{
		waveform_frame_reader_init(&state->reader, state->cur_track);
		return VOD_OK;
	}
	...
	if (state->decoder == NULL)
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0,
			"waveform_writer_init_track: vod_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0,
			"waveform_writer_init_track: vod_pool_cleanup_add failed");
		return VOD_ALLOC_FAILED;
	}
...
waveform_writer_init(
	...
	void** result)
{
	waveform_writer_state_t* state;
	vod_status_t rc;

	state = vod_alloc(request_context->pool, sizeof(waveform_writer_state_t));
	if (state == NULL)
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"waveform_writer_init: vod_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	rc = waveform_writer_init_track(state);
	if (rc != VOD_OK)
	{
		return rc;
	}
...
static vod_status_t
waveform_writer_decode_frame(audio_decoder_state_t* state, waveform_frame_t* data, int64_t* pts)
{
	AVFrame* frame;
	vod_status_t rc;
	...
	rc = waveform_calc_frame(state->request_context, frame, data);
	if (rc != VOD_OK)
	{
		return rc;
	}
...
static vod_status_t
waveform_writer_write_line(waveform_writer_state_t* state, int64_t pts)
{
	vod_status_t rc;
	int32_t rms_level;
	size_t ignore;
...
vod_status_t
waveform_writer_process(void* context)
{
	waveform_writer_state_t* state = context;
	waveform_frame_t data_buf;
	waveform_frame_t* data;
	vod_status_t rc;
	int64_t pts;
	...
	{
		// get a frame
		if (state->cur_track->media_info.codec_id == VOD_CODEC_ID_WAVEFORM)
		{
			rc = waveform_frame_reader_get_frame(&state->reader, &data, &pts);
		}
		else
		{
			data = &data_buf;
			rc = waveform_writer_decode_frame(state->decoder, &data_buf, &pts);
		}
		...
		{
			rc = waveform_writer_write_line(state, pts);
			if (rc != VOD_OK)
			{
				return rc;
			}
